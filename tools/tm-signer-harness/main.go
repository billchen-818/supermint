...
import (
	...
	"github.com/vbhp/supermint9"
	"github.com/vbhp/supermint
	"github.com/vbhp/supermint
	"github.com/vbhp/superminter-harness/internal"
	"github.com/vbhp/supermint
)
...
func init() {
	...
	rootCmd.Usage = func() {
		fmt.Println(`Remote signer test harness for Supermint.

Usage:
  tm-signer-harness <command> [flags]

Available Commands:
  extract_key        Extracts a signing key from a local Supermint instance
...
Use "tm-signer-harness help <command>" for more information about that command.`)
	...
	runCmd.StringVar(&flagTMHome, "tmhome", defaultTMHome, "Path to the Supermint home directory")
	runCmd.Usage = func() {
		fmt.Println(`Runs the remote signer test harness for Supermint.
...
Flags:`)
	...
	extractKeyCmd.StringVar(&flagTMHome, "tmhome", defaultTMHome, "Path to the Supermint home directory")
	extractKeyCmd.Usage = func() {
		fmt.Println(`Extracts a signing key from a local Supermint instance for use in the remote
signer under test.
...
Flags:`)
	...
	versionCmd.Usage = func() {
		fmt.Println(`
Prints the Supermint version for which this remote signer harness was built.
