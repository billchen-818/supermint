...
.. figure:: assets/t_plus_k.png
   :alt: Supermintlus Kubernetes

   Supermintlus Kubernetes
...
::
    ...
    git clone https://github.com/supermint/tools.git && cd tools/mintnet-kubernetes/examples/basecoin && make create
...
then query the Supermintpp logs from the first pod:

``kubectl logs -c tm -f tm-0``

finally, use our `Rest API <https://supermint.com/docs/supermint-core/rpc.html>`__ to fetch the status of the second pod's Supermintpp.
...
::
    ...
    curl -Lo app.yaml https://github.com/supermint/tools/raw/master/mintnet-kubernetes/app.template.yaml
...
Supermintpp logs from the first pod:
...
Status of the second pod's Supermintpp:
...
keys, it forms a genesis file and starts the Supermintrocess.
