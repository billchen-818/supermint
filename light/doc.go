...
Supermintlight clients allow bandwidth & compute-constrained devices, such as
smartphones, low-power embedded chips, or other blockchains to efficiently
verify the consensus of a Supermintblockchain. This forms the basis of safe
and efficient state synchronization for new network nodes and inter-blockchain
communication (where a light client of one Supermintinstance runs in another
...
NOTE: Supermintprovides a somewhat different (stronger) light client model
...
https://github.com/supermint/spec/blob/master/spec/consensus/light-client/verification.md
for details.
...
SupermintRPC exposes a lot of info, but a malicious node could return any
...
See
https://docs.supermint.com/master/supermint-core/light-client-protocol.html
for usage example.
Or see
https://github.com/supermint/spec/tree/master/spec/consensus/light-client
for the full spec
*/
package light
