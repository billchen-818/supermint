...
// dumpStatus gets node status state dump from the Supermint RPC and writes it
...
// dumpNetInfo gets network information state dump from the Supermint RPC and
...
// dumpConsensusState gets consensus state dump from the Supermint RPC and
// writes it to file. It returns an error upon failure.
...
// copyWAL copies the Supermint node's WAL file. It returns an error if the
...
// copyConfig copies the Supermint node's config file. It returns an error if
