...
var killCmd = &cobra.Command{
	...
	Short: "Kill a Supermint process while aggregating and packaging debugging data",
	Long: `Kill a Supermint process while also aggregating Supermint process data
...
Example:
$ supermint debug 34255 /path/to/tm-debug.zip`,
...
func killCmdHandler(cmd *cobra.Command, args []string) error {
	...
	logger.Info("killing Supermint process")
...
// killProc attempts to kill the Supermint process with a given PID with an
...
func killProc(pid uint64, dir string) error {
	// pipe STDERR output from tailing the Supermint process to a file
	//
	...
	// kill the underlying Supermint process and subsequent tailing process
	go func() {
		// Killing the Supermint process with the '-ABRT|-6' signal will result in
		// a goroutine stacktrace.
		p, err := os.FindProcess(int(pid))
		if err != nil {
			fmt.Fprintf(os.Stderr, "failed to find PID to kill Supermint process: %s", err)
		} else if err = p.Signal(syscall.SIGABRT); err != nil {
			fmt.Fprintf(os.Stderr, "failed to kill Supermint process: %s", err)
		}

		// allow some time to allow the Supermint process to be killed
		//
		...
		if err := cmd.Process.Kill(); err != nil {
			fmt.Fprintf(os.Stderr, "failed to kill Supermint process output redirection: %s", err)
		}
	}()
