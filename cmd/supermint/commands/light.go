...
import (
	...
	lproxy "github.com/vbhp/supermint/light/proxy"
	lrpc "github.com/vbhp/supermint/light/rpc"
	dbs "github.com/vbhp/supermint/light/store/db"
...
var LightCmd = &cobra.Command{
	Use:   "light [chainID]",
	Short: "Run a light client proxy server, verifying Supermint rpc",
	Long: `Run a light client proxy server, verifying Supermint rpc.

All calls that can be tracked back to a block header by a proof
will be verified before passing them back to the caller. Other than
that, it will present the same interface as a full Supermint node.
...
func init() {
	...
	LightCmd.Flags().StringVarP(&primaryAddr, "primary", "p", "",
		"connect to a Supermint node at this address")
	LightCmd.Flags().StringVarP(&witnessAddrsJoined, "witnesses", "w", "",
		"supermint nodes to cross-check the primary node, comma-separated")
	LightCmd.Flags().StringVar(&home, "home-dir", os.ExpandEnv(filepath.Join("$HOME", ".supermint-light")),
	...
	if primaryAddr == "" { // check to see if we can start from an existing state
		...
		if primaryAddr == "" {
			return errors.New("no primary address was provided nor found. Please provide a primary (using -p)." +
				" Run the command: supermint light --help for more information")
		}
	} else {
